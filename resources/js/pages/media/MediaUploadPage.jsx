...
export default function MediaUploadPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (files.length === 0) return toast.error('Silakan pilih minimal satu file media');
    ...
    try {
      ...
      toast.success('Media berhasil diupload ke sistem');
      navigate('/media');
    } catch (err) {
      toast.error(err.response?.data?.message || 'Gagal mengupload media. Periksa koneksi Anda.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <button onClick={() => navigate(-1)} className="text-sm font-medium text-slate-500 hover:text-emerald-600 mb-2 flex items-center transition-colors">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-extrabold text-slate-800 tracking-tight">Upload Media Baru</h2>
              <p className="text-sm font-medium text-emerald-600/80">Tambahkan dokumentasi kegiatan, rapat, atau acara ke dalam sistem</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-emerald-50 h-full flex flex-col">
              <h3 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4">Lampiran File</h3>
        ...
        <div className="flex items-center justify-end space-x-4 pt-4 border-t border-slate-200">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="inline-flex items-center gap-1.5 px-6 py-2.5 text-sm font-bold text-slate-600 bg-white border-2 border-slate-200 rounded-xl hover:bg-slate-50 hover:text-slate-800 transition-all focus:outline-none focus:ring-4 focus:ring-slate-100"
          >
